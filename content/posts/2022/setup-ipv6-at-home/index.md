...
This is the legacy version of the layer three Internet Protocol (IP).

As you may already know [IANA](https://www.iana.org/) have exhausted the IPv4 address space. The original intention of network addressing was that every device's interface would have a routable (i.e. public) address with security being handled by firewalls.

Today, we are forced to use NAT for home and mobile environments to translate the IPv4 WAN-side public IP address to internal private non-routable v4 addresses. This kind of works but is messy and problems will only increase as the number of devices in the world grows. We have already run out and some ISPs are implementing horrible solutions to get round it by using things like carrier grade NAT (CGN).

## IPv6

Almost immediately after IPv4 was rolled out, the designers realized that the number of addresses would not be enough, and began work on IPv6 with various RFCs in the late 90s. I won't go into all the history here you can [google](https://ipv6.google.com/) it. The new version of the internet protocol was [released on July 14th, 1999](https://www.iana.org/reports/1999/ipv6-announcement.html).

## What I learned about IPv6

I live in the UK and my ISP is [Zen Internet](https://www.zen.co.uk) who will provide you a massive IPv6 address range if you email them. I asked them to set it up and then spent some time to understand how this protocol works. It really does make life a lot easier. The number of Public IPs I now have though just blows my mind.
...
To test out if it's working go to [https://test-ipv6.com/](https://test-ipv6.com/).
...
The first address is called the link-local address and is generated by your PC and can only be used for machines on your local subnet. It is not routable. The `%4` on the end is the interface ID on your machine and not really part of the address.

#### Address 2: Temporary public address

`2a02:1243:5687:0:9c09:2c7a:7c78:9ffc`

When making outbound connections, the PC will rotate it's public IP for privacy reasons, although I don't really understand why this is regarded as private because the first three parts of the address are staticly assigned by my ISP, so anything after that has come from my home network. I guess it does hide which device on my home network the traffic came from.
...
This address is not used for outbound connections, but can be used for inbound connections so people can route to you from outside your network if desired. Let's say you want to run a server for example, you would use this address. This shouldn't change but if you are really paranoid you could manually configure it in your network adapter.
...
i.e. the /64 for Address 1 matches /64 for Address 3:
...
In IPv6 using ULA addresses (`FD00::/8`) will make the likelihood of address space collision very small indeed. I also wonder if companies would even bother with ULA as the global address space is so vast in number, may as well use those.

## Conclusion

Should you rush out and get IPv6 now? It pains me to say it, but you probably don't need it, but there are some great benefits
...
IPv6 has been out for nearly 25 years now and it's still not widely adopted although it is [gradually being adopted](https://www.google.com/intl/en/ipv6/statistics.html).
